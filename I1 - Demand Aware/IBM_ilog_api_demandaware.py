...
for i in range(20,21):
    ...
    #     model.add_constraint(model.sum(supply[s,w] for w in range(NbWarehouses)) == 1)                           
    ...
    # KPIs
    #objective function
    # for i in range(num_charging_points.size):
        # reward_d = reward_d + min( demandJ[i] , utility*num_charging_points[i] )
    ...
    model.minimize(-1 * (reward_d + reward_c))    
    msol=model.solve()
